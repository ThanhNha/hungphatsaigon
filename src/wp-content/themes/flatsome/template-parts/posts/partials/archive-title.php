...
<header class="archive-page-header">
	<div class="row">
		...
		if ( is_category() ) :
			...
			$terms_cate_link = get_category_link($terms_current->term_id);
			$child_categories = get_categories( array( 'parent' => $terms_current->term_id,'hide_empty' => false));
			if($child_categories){
			}else{
				if($terms_current->parent){
					$parent=get_category($terms_current->parent);
			        //$title_cate=$parent->name;
					$child_categories = get_categories( array( 'parent' => $parent->term_id));
				}
			}
			?>

			<div class="text-center text-uppercase div_subcat">
				<?php if($child_categories){
					$class="";
					if(count($child_categories)==6){
						$class="col-sm-4";
					}
					?>
					<ul class="list_cat_child nav nav-center ">
						<?php foreach($child_categories as $child_cat){?>
							<li class="<?php echo $class;?>"><a class="<?php if($terms_current->term_id==$child_cat->term_id){ echo "active";}?>" href="<?php echo  get_category_link($child_cat->term_id)?>"><?php echo $child_cat->name; ?></a></li>
						<?php } ?>
					</ul>
				<?php } ?>
			</div>
